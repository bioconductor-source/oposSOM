pipeline.sampleSimilarityAnalysisCor <- function(env)
{
  ...
  groupwise.group.colors.stability.1 <- apply((col2rgb(env$groupwise.group.colors) + 0.7 *
           (255 - col2rgb(env$groupwise.group.colors))) / 255,
        2, function(x) rgb(x[1],x[2],x[3]))
  
  groupwise.group.colors.stability.2 <- apply((col2rgb(env$groupwise.group.colors) + 0.9 *
          (255 - col2rgb(env$groupwise.group.colors))) / 255,
       2, function(x) rgb(x[1],x[2],x[3]))  
  
  
  stability.colors <- env$group.colors
  stability.colors[which(env$group.silhouette.coef < 0.5)] <- groupwise.group.colors.stability.1[env$group.labels[names(which(env$group.silhouette.coef < 0.5))]]
  stability.colors[which(env$group.silhouette.coef < 0)] <- groupwise.group.colors.stability.2[env$group.labels[names(which(env$group.silhouette.coef < 0))]]
  ...
  for (i in 1:2 )
  {
    ...
    par(mar=c(1,1,1,1))
    E(stg)$color <- apply( get.edgelist( stg ), 1, function(x) mixColors( env$group.colors[x[1]], env$group.colors[x[2]] ) )
    ...
    plot(stg, layout=layout, vertex.size=5, vertex.label = rep("",ncol(adj.matrix)),
      ...
      legend("bottomright", rep(as.character(unique(env$group.labels)),3), cex=0.5,
             text.col=c(env$groupwise.group.colors,groupwise.group.colors.stability.1,groupwise.group.colors.stability.2), bg="white",ncol=3,
  ...
  for (i in 1:2 )
  {
    ...
    if (max(adj.matrix) > 0)
    {
      ...
      par(mar=c(1,1,1,1))
      E(g)$color <- apply( get.edgelist( g ), 1, function(x) mixColors( env$group.colors[x[1]], env$group.colors[x[2]] ) )
      plot(g, layout=layout, vertex.size=5, vertex.label = rep("",ncol(adj.matrix)),
          vertex.color=env$group.colors, main=paste("Correlation backbone (2NN-graph) on",n))
        legend("bottomright", as.character(unique(env$group.labels)), cex=0.5, text.col=env$groupwise.group.colors, bg="white")
        box()
      ...
      if (ncol(adj.matrix) < 1000)
      {
        plot(g, layout=layout, vertex.size=5, vertex.label = colnames(adj.matrix),
            vertex.label.cex=if (ncol(adj.matrix)<100) 1.2 else 0.6,
            vertex.color=env$group.colors, main=paste("Correlation backbone on",n))
         legend("bottomright", as.character(unique(env$group.labels)), cex=0.5, text.col=env$groupwise.group.colors, bg="white")
         box()
      }
    }
  ...
  for (i in 1:2 )
  {
    ...
    par(mar=c(1,1,1,1))
    heatmap(x=d, zlim=c(-1,1), Rowv=NA, Colv=NA, col=env$color.palette.heatmaps(1000),
                ...
                margins=c(8,6), ColSideColors=env$group.colors, RowSideColors=env$group.colors)
    ...
    par(mar=c(1,1,1,1))
    heatmap(x=d[o,o], zlim=c(-1,1), Rowv=NA, Colv=NA, col=env$color.palette.heatmaps(1000),
                  labRow=if(nrow(d)<100) rownames(d[o]) else rep("",nrow(d)),
                  labCol=if(ncol(d)<100) colnames(d[o]) else rep("",ncol(d)),
                  scale="n", main=paste("Pairwise correlation map on",n),
                  margins=c(8,6), ColSideColors=env$group.colors[o], RowSideColors=env$group.colors[o])
    ...
    if (ncol(d) < 1000 && i == 1)
    {
      ...
      pipeline.moduleCorrelationMap(env, d.noNA, hcl)
